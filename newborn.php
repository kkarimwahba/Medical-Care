...
if ($connect->connect_error) {
  ...
  if(isset($_POST["book"]))
  {
    ...
    {
      $doctor_array_id = array_column($_SESSION["cart3"], "doctor_id");
      if(!in_array($_GET["id"], $doctor_array_id))
      {
        $count = count($_SESSION["cart3"]);
        $doctor_array = array(
          'doctor_id'			=>	$_GET["id"],
        ...
        $_SESSION["cart3"][$count] = $doctor_array;
      }
      else
      {
  ...
  if(isset($_GET["action"]))
  {
    ...
    {
      foreach($_SESSION["cart3"] as $keys => $values)
      {
        if($values["doctor_id"] == $_GET["id"])
        {
          unset($_SESSION["cart3"][$keys]);
          echo '<script>alert("doctor Removed")</script>';
          echo '<script>window.location="newborn.php"</script>';
        }
      }
    }
  } 
...
<html>
<head>
  <title>Pediatrics and New Born</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="styles.css">
</head>
<body>
<section class="book">
<h1>Pediatrics and New Born</h1>
<br><br>
<div class="row">
...
<form method="POST" action="newborn.php?action=add&id=<?php echo $row["id"]; ?>">
...
<h3 class="doctor">Book Details</h3><br>
            ...
            <td><?php echo $values["doctor_date"]; ?></td>
						<td><a href="newborn.php?action=delete&id=<?php echo $values["doctor_id"]; ?>"><span class="text-danger">Remove</span></a></td>
					</tr>
					<?php
          $fees = $fees + ($values["doctor_fees"]);
			...
			<?php 
				if(isset($_POST["submit"])){
				 foreach($_SESSION['cart3'] as $array){
