...
if ($connect->connect_error) {
  ...
  if(isset($_POST["book"]))
  {
    ...
    {
      $doctor_array_id = array_column($_SESSION["cart4"], "doctor_id");
      if(!in_array($_GET["id"], $doctor_array_id))
      {
        ...
        $_SESSION["cart4"][$count] = $doctor_array;
      }
      else
      {
...
<form method="POST" action="bones.php?action=add&id=<?php echo $row["id"]; ?>">
...
<h3 class="doctor">Book Details</h3><br>
            ...
            <td><?php echo $values["doctor_date"]; ?></td>
						<td><a href="bones.php?action=delete&id=<?php echo $values["doctor_id"]; ?>"><span class="text-danger">Remove</span></a></td>
					</tr>
					<?php
          $fees = $fees + ($values["doctor_fees"]);
			...
			<?php 
				if(isset($_POST["submit"])){
				 foreach($_SESSION['cart4'] as $array){
