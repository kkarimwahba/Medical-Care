...
if ($connect->connect_error) {
  ...
  if(isset($_POST["book"]))
  {
    ...
    {
      $doctor_array_id = array_column($_SESSION["cart2"], "doctor_id");
      if(!in_array($_GET["id"], $doctor_array_id))
      {
        ...
        $_SESSION["cart2"][$count] = $doctor_array;
      }
      else
      {
  ...
  if(isset($_GET["action"]))
  {
    if($_GET["action"] == "delete")
    {
      foreach($_SESSION["cart2"] as $keys => $values)
      {
        ...
        {
          unset($_SESSION["cart2"][$keys]);
          echo '<script>alert("doctor Removed")</script>';
          echo '<script>window.location="heart.php"</script>';
        }
      }
    }
  } 
...
<html>
<head>
  <title>Cardiology and Vascular Disease</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="styles.css">
</head>
<body>
<section class="book">
<h1>Cardiology and Vascular Disease</h1>
...
if(mysqli_num_rows($result) > 0)
{
  ...
  {
?>
<form method="POST" action="heart.php?action=add&id=<?php echo $row["id"]; ?>">
...
<h3 class="doctor">Book Details</h3><br>
            ...
            <td><?php echo $values["doctor_date"]; ?></td>
						...
						<td><a href="heart.php?action=delete&id=<?php echo $values["doctor_id"]; ?>"><span class="text-danger">Remove</span></a></td>
					</tr>
					<?php
          $fees = $fees + ($values["doctor_fees"]);
			...
			<?php 
				if(isset($_POST["submit"])){
				 foreach($_SESSION['cart2'] as $array){
