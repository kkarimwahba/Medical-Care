...
if ($connect->connect_error) {
  ...
  if(isset($_POST["book"]))
  {
    ...
    {
      $doctor_array_id = array_column($_SESSION["cart6"], "doctor_id");
      if(!in_array($_GET["id"], $doctor_array_id))
      {
        $count = count($_SESSION["cart6"]);
        $doctor_array = array(
          'doctor_id'			=>	$_GET["id"],
        ...
        $_SESSION["cart6"][$count] = $doctor_array;
      }
      else
      {
  ...
  if(isset($_GET["action"]))
  {
    ...
    {
      foreach($_SESSION["cart6"] as $keys => $values)
      {
        if($values["doctor_id"] == $_GET["id"])
        {
          unset($_SESSION["cart6"][$keys]);
          echo '<script>alert("doctor Removed")</script>';
          echo '<script>window.location="diet.php"</script>';
        }
      }
    }
  }  
...
<form method="POST" action="diet.php?action=add&id=<?php echo $row["id"]; ?>">
...
<h3 class="doctor">Book Details</h3><br>
          ...
          <td><?php echo $values["doctor_time"]; ?></td>
            <td><?php echo $values["doctor_date"]; ?></td>
						<td><a href="diet.php?action=delete&id=<?php echo $values["doctor_id"]; ?>"><span class="text-danger">Remove</span></a></td>
					</tr>
					<?php
          $fees = $fees + ($values["doctor_fees"]);
			...
			<?php 
				if(isset($_POST["submit"])){
				 foreach($_SESSION['cart6'] as $array){
